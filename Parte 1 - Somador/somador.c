...
int main(int argc, char** argv) {
    ...
    long int chunk_size, vector_length; 
    ...
    if (argc != 3) {
        printf("Uso: %s <vector_length> <n_threads>\n", argv[0]);
        exit(EXIT_FAILURE);
    }

    vector_length = atoi(argv[1]);
    n_threads = atoi(argv[2]);
    chunk_size = floor(vector_length / n_threads);
    n_overloaded_threads = vector_length % n_threads;


    // Aloca o vetor de números aleatórios
    numbers = (int8_t*) malloc(vector_length * sizeof(int8_t));
    if (numbers == NULL) {
    ...
    for (long int i = 0; i < vector_length; i++) {
    ...
    elapsed_time = ((double)(end_time - start_time) / CLOCKS_PER_SEC) * 1e3;
